...
struct SettingsView: View {
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 30) {
                            VStack(alignment: .leading, spacing: 6) {
                                Text("Settings").font(.system(size: 28, weight: .bold)).foregroundColor(appState.darkMode ? .white : .primary)
                                Text("Manage your account, notifications and preferences").foregroundColor(appState.darkMode ? .gray : .secondary)
                            }
                            .padding(.top, 30).padding(.horizontal, 40)
                            
                            if showSaveSuccess {
                                Text("✓ Settings saved successfully!").foregroundColor(.green).font(.subheadline).padding().frame(maxWidth: .infinity).background(Color.green.opacity(0.1)).cornerRadius(8).transition(.opacity)
                            }
                            ...
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 24) {
                                SettingsCard(title: "Edit Profile", icon: "person", darkMode: appState.darkMode) {
                                    VStack(spacing: 16) {
                                        ...
                                        Button(action: saveProfile) {
                                            Text("Save Changes").fontWeight(.semibold).foregroundColor(.white).frame(maxWidth: .infinity).padding(.vertical, 12).background(Color.orange).cornerRadius(8)
                                        }.padding(.top, 8)
                                    }
                                }
                                ...
                                SettingsCard(title: "Change Password", icon: "lock.shield", darkMode: appState.darkMode) {
                                    VStack(spacing: 16) {
                                        ...
                                        if showPasswordError { Text(passwordError).foregroundColor(.red).font(.subheadline).frame(maxWidth: .infinity, alignment: .leading) }
                                        Button(action: updatePassword) {
                                            Text("Update Password").fontWeight(.semibold).foregroundColor(.white).frame(maxWidth: .infinity).padding(.vertical, 12).background(Color.orange).cornerRadius(8)
                                        }.padding(.top, 8)
                                    }
                                }
    ...
    private func updatePassword() {
        if newPassword != confirmPassword { passwordError = "New passwords do not match"; showPasswordError = true; return }
        if newPassword.count < 6 { passwordError = "Password must be at least 6 characters"; showPasswordError = true; return }
        if currentPassword.isEmpty { passwordError = "Please enter your current password"; showPasswordError = true; return }
        passwordError = ""; showPasswordError = false; currentPassword = ""; newPassword = ""; confirmPassword = ""
        showSaveSuccess = true
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) { showSaveSuccess = false }
    }
}
...
struct SettingsCard<Content: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            HStack(spacing: 10) {
                Image(systemName: icon).foregroundColor(.orange).font(.system(size: 18))
                Text(title).font(.system(size: 18, weight: .bold)).foregroundColor(darkMode ? .white : .primary)
            }
            content
        }
        .padding(28).background(darkMode ? Color(red: 0.15, green: 0.17, blue: 0.2) : Color.white).cornerRadius(12).shadow(color: .black.opacity(0.04), radius: 3, y: 1)
    }
}

struct SettingsTextField: View {
    let title: String; @Binding var text: String; var darkMode: Bool = false
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(title).font(.system(size: 13, weight: .semibold)).foregroundColor(darkMode ? Color(red: 0.7, green: 0.7, blue: 0.75) : Color(red: 0.4, green: 0.4, blue: 0.45))
            TextField("", text: $text).textFieldStyle(.plain).padding(12)
                .background(RoundedRectangle(cornerRadius: 8).fill(darkMode ? Color(red: 0.08, green: 0.09, blue: 0.11) : Color.white))
                .overlay(RoundedRectangle(cornerRadius: 8).stroke(darkMode ? Color(red: 0.3, green: 0.32, blue: 0.35) : Color(red: 0.85, green: 0.86, blue: 0.88), lineWidth: 1))
...
struct SettingsSecureField: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(title).font(.system(size: 13, weight: .semibold)).foregroundColor(darkMode ? Color(red: 0.7, green: 0.7, blue: 0.75) : Color(red: 0.4, green: 0.4, blue: 0.45))
            SecureField("", text: $text).textFieldStyle(.plain).padding(12)
                .background(RoundedRectangle(cornerRadius: 8).fill(darkMode ? Color(red: 0.08, green: 0.09, blue: 0.11) : Color.white))
                .overlay(RoundedRectangle(cornerRadius: 8).stroke(darkMode ? Color(red: 0.3, green: 0.32, blue: 0.35) : Color(red: 0.85, green: 0.86, blue: 0.88), lineWidth: 1))
                .foregroundColor(darkMode ? .white : .black)
        }
...
struct ToggleRow: View {
    let title: String; @Binding var isOn: Bool; var isLast: Bool = false; var darkMode: Bool = false
    var body: some View {
        HStack {
            Text(title).font(.system(size: 14)).foregroundColor(darkMode ? .white : .primary)
            Spacer()
